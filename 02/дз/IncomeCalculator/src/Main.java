...
public class Main {
    ...
    public static void main(String[] args) {
        //рассчет минимальной суммы дохода для инвестирования
        int minIncomeForInvest = minIncome;
        while(true)
        {
            double sum  = (minIncomeForInvest - minIncomeForInvest * managerPercent - calculateFixedCharges()) -
                    ((minIncomeForInvest - minIncomeForInvest * managerPercent - calculateFixedCharges()) * mainTaxPercent);
            if (sum <= 100000) minIncomeForInvest++;
            else break;
        }

        while (true) {
            System.out.println("Введите сумму доходов (минимальная сумма дохода для инвестирования: " + minIncomeForInvest + " руб.) компании за месяц " +
